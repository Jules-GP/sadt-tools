...
| [DOCShapeAXI](tools/DOCShapeAXI/) | `DOCShapeAXI/`, `DOCShapeAXI_CLI/` | `ea37083` (2026-08-14) | 2026-08-20 | no -- repackaging only. The prediction and the GradCAM are upstream's step for step. Three behaviours changed and each is a correction: the checkpoint is named by the caller instead of downloaded from a GitHub release at run time, the manifest is rebuilt rather than appended to (upstream re-grades every surface when an output folder is reused), and a task with no model of its own is refused instead of being answered with the four-class model. Shares Crown_Seg's torch 2.11 / pytorch3d pairing rather than adding a third runtime. **No real grading run yet** -- it needs the `grading` extra and the hosted bundle. |

...
| [AutoMatrix](tools/AutoMatrix/) | `Automatrix_CLI/`, `AutoMatrix/` | `9318adb` (2026-07-20) | 2026-08-20 | no -- repackaging only: the resampling and the landmark transform are upstream's, argument for argument, and pinned to SimpleITK 2.5.6 where upstream pins nothing. Four behaviours around them changed and each is in the tool README: the "From AReg" branch reads the matrix folder instead of an argument the CLI never declares (it raised AttributeError upstream), a missing reference volume is refused rather than warned about, surface meshes and `.npy` matrices are refused by name rather than failing into a log, and a batch that wrote nothing raises instead of returning an empty folder. Validated against synthetic geometry with closed-form answers; **not** diffed against the pre-port CLI on a real cohort. |

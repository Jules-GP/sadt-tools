...
| [MRI2CBCT](tools/MRI2CBCT/) | `MRI2CBCT/`, `MRI2CBCT_CLI/` | `6d8e49b` (2026-07-17) | 2026-08-20 | no -- repackaging only. The ported modules are upstream's files; what changed is the import mechanics, the logging, and two defects that stopped them running here at all (an f-string only Python 3.12 parses, and `torchreg` imported but never declared). Upstream's pydicom 2.2.2 / dicom2nifti 2.3.0 / cu118 torch install is gone with the shared site-packages it targeted. Orient, Resample and LR crop are validated against known geometry; **Approximate, TMJ crop and Register are not run** -- the first two need the nnUNet condyle bundle and the third needs a clinical pair. |

...
def find_pairs(t1: Path, t2: Path, masks: Path = None, inits: Path = None) -> list:
    ...
    * two FILES, taken as the one pair, which is what the panel's fixed/moving
    ...
    masks_by_key = _indexed(Path(masks), VOLUME_SUFFIXES) if masks else {}
    inits_by_key = _indexed(Path(inits), (MATRIX_SUFFIX,)) if inits else {}

    if t1.is_file():
        key = patient_key(t1.name) or t1.name.split(".")[0]
        return [(key, t1, t2, masks_by_key.get(key), inits_by_key.get(key))]
